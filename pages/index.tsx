...
const fetchRepositories = async () => {
	const repos = await axios.get(`https://api.github.com/users/mauriziocarella/repos`)
		.then((res) => res.data)
		.then((repos): RepositoryType[] => repos.map((repo: any) => ({
			...repo,
			url: repo.html_url,
...
const Social: React.VFC<{social: typeof socials[number]}> = ({social, ...props}) => (
	<a rel="noopener noreferrer" href={social.link} className="tooltip p-2" target="_blank" aria-label={social.label} data-tip={social.label}>
		{social.icon}
	</a>
)
...
const Repository: React.VFC<{repository: RepositoryType} & React.HTMLProps<HTMLDivElement>> = ({repository, className, ...props}) => {
	return (
				...
				<div className="flex-1 flex flex-col flex-grow">
					<div className="p-2 flex items-center absolute top-0 w-full md:static">
						...
						<div className="tooltip tooltip-bottom tooltip-left" data-tip="Click to toggle theme">
							<button type="button" className="btn btn-ghost" onClick={() => toggleTheme()} aria-hidden={true} aria-label="Toggle theme">
					...
					<Scrollbar className="flex-grow md:overflow-y-auto p-4">
						...
						<div className="text-center my-6 italic text-sm">
							<a rel="noopener noreferrer" href="https://github.com/mauriziocarella" target="_blank">
